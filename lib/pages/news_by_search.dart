...
class NewsBySearch extends StatefulWidget {
  String searchText;
  NewsBySearch({super.key, required this.searchText});
...
class _NewsBySearchState extends State<NewsBySearch> {
  ...
  void initState() {
    loadNews();
    loadNativeAds();
    textInpControllerSearch.text = widget.searchText;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Scaffold(
        appBar: AppBarCustom(appBarTitle: Strings.search, isSettings: true),
        backgroundColor: Colors.transparent,
        body: Column(
          children: [
            Padding(
              padding: EdgeInsets.fromLTRB(15, 10, 15, 0),
              child: Methods.getEditTextInputSearch(
                context,
                textInpControllerSearch,
                true,
                () {
                  setState(() {
                    pageNumber = 0;
                    isOver = false;
                    listNews.clear();
                    widget.searchText = textInpControllerSearch.text;
                    loadNews();
                  });
                },
              ),
  ...
  // Future<List<ItemNews>?> loadNews() async {
  //   pageNumber = pageNumber + 1;
  //   return await new ApiCLient().getNewsBySearch(Constants.METHOD_NEWS_BY_SEARCH, SharedPref.getUserID().toString(), widget.searchText, pageNumber);
  // }
    ...
    if (await Methods.checkConnectivity()) {
      ...
      try {
        List<ItemNews>? tempList =
            await new ApiCLient().getNewsBySearch(Constants.METHOD_NEWS_BY_SEARCH, SharedPref.getUserID().toString(), widget.searchText, pageNumber);
        if (tempList != null) {
