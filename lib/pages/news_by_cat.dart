...
class NewsByCat extends StatefulWidget {
  ...
  NewsByCat({super.key, required this.catID, required this.catName, required this.isFromNotification});

  @override
  State<NewsByCat> createState() => _NewsByCatState();
}
...
class _NewsByCatState extends State<NewsByCat> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Scaffold(
        appBar: AppBarCustom(appBarTitle: widget.catName, isSettings: true, isFromNotification: widget.isFromNotification),
    ...
    if (await Methods.checkConnectivity()) {
      ...
      try {
        List<ItemNews>? tempList =
            await new ApiCLient().getNewsByCat(Constants.METHOD_NEWS_BY_CAT, SharedPref.getUserID().toString(), widget.catID, pageNumber);
        if (tempList != null) {
